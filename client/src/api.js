...
export const api = {
  ...
  getMonthly: (month) => request(`/api/monthly?month=${encodeURIComponent(month)}`),
  saveMonthlyExpense: (payload) => request('/api/monthly/expenses', { method: 'PUT', body: JSON.stringify(payload) }),
  deleteMonthlyExpense: (id, month) => request(`/api/monthly/expenses/${id}?month=${encodeURIComponent(month)}`, { method: 'DELETE' }),
  updateRecurringExpense: (id, payload) => request(`/api/monthly/recurring/${id}`, { method: 'PUT', body: JSON.stringify(payload) }),
